//exercise: look for properties, in NY or not in NY
var readline = require('readline')

var options = 'list, show n, reserve, occupancy n max, search amenity, find location';
...
function search(amenity) {
    ...
    for (var i = 0; i < rooms.length; i++) {
        if (rooms[i].amenities.includes(amenity)) { // if(!rooms[i].reserved && rooms[i].amenities.indexof(amenity) > -1) another correct way
            var counter = i + 1;
            console.log(
                padLeft(counter.toString(), 2),
                '...',
...
rl.on('line', function (input) {
    ...
    if (inputArr[0] === 'list') {
        ...
        search(inputArr.slice(1).join(' ')); // make sure when finding amenities that are more than one word long
    } else {
...
function find(location) {
    ...
    for (var i = 0; i < rooms.length; i++) {
        if (rooms[i].location.includes(location)) { 
            var counter = i + 1;
            console.log(
                padLeft(counter.toString(), 2),
                '...',
